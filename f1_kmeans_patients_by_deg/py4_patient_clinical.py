...
#    if pvalue<1e-7:
    ...
    # survival curves
        ...
        g1 = kmf_control.fit(t1, e1).plot(ax=ax,show_censors=True,label='patient G1',\
                        censor_styles={'ms': 12, 'marker': '+'},ci_show=False,c='k',ls='-')
    
        kmf_exp = KaplanMeierFitter()
        #g2 = kmf_exp.fit(t2, e2, label=legends[1]).plot(ax=ax,show_censors=True,\
        g2 = kmf_exp.fit(t2, e2).plot(ax=ax,show_censors=True,label='patient G2',\
                    censor_styles={'ms': 12, 'marker': '+'},ci_show=False,c='silver',ls='-')
        handles, labels = ax.get_legend_handles_labels()
        label_a_index = labels.index('patient G1')
        label_b_index = labels.index('patient G2')
...
indirs = ['f1_kmeans_PAML_patients_by_PAML_DEG','f2_kmeans_SG_patients_by_PAML_DEG','f3_kmeans_SG_patients_by_PAML_SG_DEG']
indir_cohorts=['PAML','SG','SG']
outdir = 'f4_patient_clinical'
os.makedirs(outdir,exist_ok=True)
    

clinical_results = pd.DataFrame()   
    
for ii in np.arange(len(indirs)):  
    indir=indirs[ii]
    cohort=indir_cohorts[ii]  
    fig_basename = indir[3:]
    clinical_df = return_patient_info(cohort)
    basename = '{}_pthre5_rk3_ck2'.format(cohort)
    patient_c1 = [i.strip() for i in open(indir+'/txt/{}_patients_cluster1.txt'.format(basename)).readlines()]
    patient_c2 = [i.strip() for i in open(indir+'/txt/{}_patients_cluster2.txt'.format(basename)).readlines()]
#     if cohort=="PAML":
#         patient_c2 = [i.strip() for i in open(indir+'/txt/{}_patients_cluster1.txt'.format(basename)).readlines()]
#         patient_c1 = [i.strip() for i in open(indir+'/txt/{}_patients_cluster2.txt'.format(basename)).readlines()]
    # ==== change to AML_xxx ID
#     clinical_dir = '/nv/vol190/zanglab/zw5j/wwork2018/AML_fran/f0_fran_data_new/data_processed/'
#     info = pd.read_csv(clinical_dir+os.sep+'{}_clinical.csv'.format(cohort),index_col=0)
#     patient_c1 = info.loc[patient_c1]['subject_ID']
#     patient_c2 = info.loc[patient_c2]['subject_ID']
