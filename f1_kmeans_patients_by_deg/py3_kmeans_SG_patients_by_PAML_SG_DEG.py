...
def kmeans_clustering_index(df,rk,ck,t_times,basename,outdir):

    df_newindex,df_newindex_sep = return_index_of_consistent_kmeans(df,rk,r_times=t_times)
    df_newcol,df_newcol_sep = return_index_of_consistent_kmeans(np.transpose(df),ck,r_times=t_times)    
...
def main():
    ...
    outdir='f3_kmeans_SG_patients_by_PAML_SG_DEG'
    os.makedirs(outdir,exist_ok=True)
    os.makedirs(outdir+os.sep+'csv',exist_ok=True)
    
    file_dir = '/nv/vol190/zanglab/zw5j/wwork2018/AML_fran/f0_fran_data_new/data_processed'
    # == this is DEG of PAML 
    paml_genes1 = [i.strip() for i in open('f1_kmeans_PAML_patients_by_PAML_DEG/txt/PAML_pthre5_rk3_ck2_genes_cluster1.txt').readlines()]
    paml_genes2 = [i.strip() for i in open('f1_kmeans_PAML_patients_by_PAML_DEG/txt/PAML_pthre5_rk3_ck2_genes_cluster2.txt').readlines()]
    paml_genes3 = [i.strip() for i in open('f1_kmeans_PAML_patients_by_PAML_DEG/txt/PAML_pthre5_rk3_ck2_genes_cluster3.txt').readlines()]
    selected_genes = paml_genes1+paml_genes2+paml_genes3
...
#             logFC_ori = pd.read_csv(file_dir+os.sep+'{}_deseq2_log2FC.txt'.format(cohort),sep=',',index_col=0)
    
            df = logFC_ori.loc[logFC_ori.index.intersection(selected_genes)]
            t_times = 20
            for rk in [3]:
                for ck in [2]:
                    basename = '{}_pthre{}_rk{}_ck{}'.format(cohort,patient_thre,rk,ck);print('\n\n',basename,'\n')
                    newindex,newcol,counts_row,counts_col = kmeans_clustering_index(df,rk,ck,t_times,basename,outdir)
