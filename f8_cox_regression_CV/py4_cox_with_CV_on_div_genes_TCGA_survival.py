...
# == main ==
outdir='f4_cox_CV_on_div_genes_TCGA_survival'
...
selected_genes = tcga_AML_expression_df.index.intersection(selected_genes).intersection(tcga_AML_clinical_df.columns)
tcga_AML_data=np.transpose(tcga_AML_expression_df.loc[selected_genes])

inner_product=np.inner(tcga_AML_data,cph.summary['coef'].loc[selected_genes])
