...
outdir = 'f3_RP_compr_enrichr_genes_figs'
...
selected_terms = ['Osteoclast differentiation','Cell cycle','DNA replication']
cohort='PAML'
...
for group_key in list(groups_matches.keys())[:]:
    ...
    report_file = 'enrichr/{}/KEGG_2019_Human.enrichr.enrichr.reports.txt'.format(group_key)
    report_df = pd.read_csv(report_file,sep='\t',index_col=0)
    for term in report_df.index[:5]:
        if not term in selected_terms:
            continue
...
g1 = sns.heatmap(np.transpose(-1*np.log10(ttest_df_p)),cmap = plt.cm.Reds,xticklabels=True, yticklabels=True,\
                cbar=True,vmin=0,vmax=10,\
                cbar_kws={"ticks":[0,5,10]})#"use_gridspec":False,"location":"top"})    
plt.savefig('{}/stats_p.pdf'.format(outdir),bbox_inches='tight',pad_inches=0.1,dpi=600,transparent=True)
plt.show()
plt.close()
