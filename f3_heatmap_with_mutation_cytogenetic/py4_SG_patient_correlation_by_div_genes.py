...
def plot_correlation_figs(cohort,outdir,project_dir,deg_clustering_dir,patient_clustering_dir,flag):
    ...
    if flag=='div_genes':
        div_genes = [i.strip() for i in open(deg_clustering_dir+'/txt/PAML_pthre5_rk3_ck2_genes_cluster3.txt'.format(basename)).readlines()]
        div_genes = [i for i in div_genes if i in deg_df.index]
...
outdir = 'f4_SG_patient_correlation'
