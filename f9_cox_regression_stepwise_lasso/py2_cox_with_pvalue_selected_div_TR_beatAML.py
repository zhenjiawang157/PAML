...
# == main ==
outdir='f2_cox_on_pvalueThre_div_TR_beatAML_survival'
...
for p_thre in p_thres[:]:
    ...
    # cph = CoxPHFitter()
    cph = CoxPHFitter(penalizer=0.1, l1_ratio=1) # sparse solutions,
    ...
    selected_genes = cph.summary['coef'][np.abs(cph.summary['coef'])>1e-6].sort_values(ascending=False).index
    # plot log HR
    ...
    figname=outdir+os.sep+'Pthre{}_clinical_by_cox.png'.format(p_thre)
    results = survival_for_two(survival_df,'c1','c2',figname,p_thre)
    survival_df.to_csv('{}/Pthre{}_clinical_by_cox_info.csv'.format(outdir,p_thre))
