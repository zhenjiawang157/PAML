...
def survival_for_two(df,treat,ctrl,figname,risk_days,flag,gene_count):
    ...
    plt.axes().text(df['time'].max()*0.85,0.05,'p={:.1e}'.format(pvalue),fontsize=12,ha='center')
    plt.ylim([-0.02,1.05])
#     plt.xlim([0,max_val*1])
    plt.title('{}\n{} w/ {} genes'.format(basename.split('Cox_')[1],flag,gene_count),fontsize=15)
...
def calinical_analysis(summary_score,cph,logrank_expr_data,logrank_clinical_data,logrank_clinical_cols,flag,basename,risk_days,gene_count):
    ## ==== survival anlaysis
    ...
    results = survival_for_two(survival_df,'c1','c2',figname,risk_days,flag,gene_count)
...
# == main ==
outdir='f7_cox_on_random_fewer_genes_beatAML_survival'
...
for gene_count in [5,10,15,20][:]:
    # assign index accroding to the number of genes
    summary_score=pd.DataFrame()
    rand_gene_df = pd.DataFrame(index = np.arange(gene_count))
    for group in ['groupA','groupB','groupC'][:]:
        for rand_time in np.arange(5):
            # random select few genes
            group_genes = beatAML_expression_df.index.intersection(deg_dict[group])
            random_genes = np.random.choice(group_genes,gene_count,replace=False)
            ...
            for penalizer in penalizers[:]:
                for l1_ratio in l1_ratios[:]:
                    basename = '{}_geneCount{}_random{}_Cox_penalizer{}_l1_ratios{}'.format(group,gene_count,rand_time,'p'.join(str(penalizer).split('.')),'p'.join(str(l1_ratio).split('.')))
                    ...
                    calinical_analysis(summary_score,cph,beatAML_data,beatAML_clinical_df,logrank_clinical_cols,'BeatAML',basename,risk_days,gene_count)
                        
    ## == save out the results
    summary_score.to_csv('{}/summary_score_geneCount{}.csv'.format(outdir,gene_count))        
    rand_gene_df.to_csv('{}/random_genes_geneCount{}.csv'.format(outdir,gene_count))      
