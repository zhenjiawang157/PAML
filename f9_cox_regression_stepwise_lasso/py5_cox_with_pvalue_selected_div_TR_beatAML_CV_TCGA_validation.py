...
def survival_for_two(df,treat,ctrl,figname,thre,risk_days,flag):
...
def calinical_analysis(summary_score,cph,logrank_expr_data,logrank_clinical_data,logrank_clinical_cols,flag,basename,risk_days):
    ...
    results = survival_for_two(survival_df,'c1','c2',figname,p_thre,risk_days,flag)
...
# == main ==
outdir='f5_cox_on_pvalueThre_div_TR_beatAML_survival_CV_TCGA_validation'
...
for penalizer in penalizers[:]:
    for l1_ratio in l1_ratios[:]:
        basename = 'TR_Pthre{}_Cox_penalizer{}_l1_ratios{}'.format(p_thre,'p'.join(str(penalizer).split('.')),'p'.join(str(l1_ratio).split('.')))
        ...
        ## ==== survival anlaysis of TCGA data
        selected_genes = tcga_AML_expression_df.index.intersection(selected_genes)
        tcga_AML_data=np.transpose(tcga_AML_expression_df.loc[selected_genes])
        risk_days = [0,500,1000,1500,2000,2500,3000]
        logrank_clinical_cols = ['vital_status','time_max']
        calinical_analysis(summary_score,cph,tcga_AML_data,tcga_AML_clinical_df,logrank_clinical_cols,'TCGA',basename,risk_days)
