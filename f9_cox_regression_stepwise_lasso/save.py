...
def survival_for_two(df,treat,ctrl,figname):
    ...
    plt.axes().text(df['time'].max()*0.75,0.05,'p={:.2e}'.format(pvalue),fontsize=14,ha='center')
    plt.ylim([-0.02,1.05])
...
# == main ==
outdir='f3_cox_CV_on_div_genes_beatAML_survival'
...
# bart_results = pd.read_csv('{}/updated_202010/f4_bart2_TR_expression/f2_PAML_DEG_DMG_removed_bart2/f1_DMG_removed_DEG_bart2_results/PAML_pthre5_rk3_ck2_genes_cluster3_div_bart_results.txt'.format(project_dir),sep='\t',index_col=0)
# div_genes = bart_results[bart_results['irwin_hall_pvalue']<0.01].index
...
beatAML_data=np.transpose(beatAML_expression_df.loc[div_genes].dropna()) # gene on the column
...
k=5
cph_data_cv = pd.concat([beatAML_clinical_df,beatAML_data[selected_genes]],axis=1)
cv_score=pd.DataFrame(columns=np.arange(k))
cph_cv = CoxPHFitter()
for ii in np.arange(10):
    scores = k_fold_cross_validation(cph_cv, cph_data_cv,'overallSurvival',event_col='vitalStatus',k=k,scoring_method="concordance_index")
