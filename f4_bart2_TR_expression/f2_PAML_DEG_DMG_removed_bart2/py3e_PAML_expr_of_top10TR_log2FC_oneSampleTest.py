...
def expr_plot_for_patients(top_tfs,patients1,patients2,log2FC,cohort,group,outdir):
    ...
    for ii in np.arange(len(top_tfs)):
        ...
        box_val1 = log2FC.loc[gene_name][patients1].values
        box_val2 = log2FC.loc[gene_name][patients2].values
        box_vals1.append(box_val1)
        box_vals2.append(box_val2)
        gene_names.append(gene_name)


        s1,p1 = stats.ttest_1samp(box_val1,0)
        s2,p2 = stats.ttest_1samp(box_val2,0)
        sig1_marker.append('*' if p1<0.05 else '')
        sig2_marker.append('*' if p1<0.05 else '')
...
# ==== main section
outdir = 'f3e_PAML_expr_of_top10TR_log2FC_oneSampleTest'
