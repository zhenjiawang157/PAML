...
def mark_pvalue(compr_pos,positions,box_vals):
    ...
    y, h, col = np.percentile(np.append(box_vals[compr_pos[0]],box_vals[compr_pos[1]]),95)*1.00 ,1.02, 'k'
...
def compr_boxplot(positions,box_vals,figname,top_tfs,colors,cohort,group,flag):
    ...
    plt.figure(figsize=(3.7,2.9))
    g = plt.boxplot(box_vals,positions=positions,widths = .4,patch_artist=True,\
...
#             tf_expr_compr_ttest.loc[top_tfs[i],'pvalue'] = p
    ...
    plt.axes().set_xticks(np.arange(len(top_tfs)))
...
#     plt.legend([g["boxes"][0],g["boxes"][1]],['Diagnosis','Relapse'],fontsize=12,borderaxespad=0.2,labelspacing=.2,handletextpad=0.2,handlelength=1,loc=0,frameon=False)
    plt.ylabel('log2 foldchange',fontsize=16)
...
def expr_plot_for_patients(top_tfs,patients,log2FC,cohort,group,flag,outdir):
    ...
    top_tfs = [i for i in top_tfs if i in g1_sig_tfs.union(g2_sig_tfs).union(['KMT2A'])]
    print(top_tfs)
    ...
    for ii in np.arange(len(top_tfs)):
        ...
        box_vals.append(log2FC.loc[gene_name][patients].values)
        # relapse
#         positions.append(ii+.25)
#         box_vals.append(tpm_relapse.loc[gene_name][patients].values)
        gene_names.append(gene_name)
    if flag=='G1':
        colors = ['tab:orange']*top_tf_num
    else:
        colors = ['tab:blue']*top_tf_num
    figname = '{}/{}_{}_patient{}.pdf'.format(outdir,cohort,group,flag)
    compr_boxplot(positions,box_vals,figname,gene_names,colors,cohort,group,flag)
...
# ==== main section
outdir = 'f3b_PAML_expr_of_topTR_sep_patient_sig_log2FC'
...
#         print('\n',top_tfs.difference(tpm_diagnosis.index))
        ...
        # for patient group 1
        expr_plot_for_patients(top_tfs,patient_list1,log2FC,cohort,group,'G1',outdir)

        # for patient group 1
        expr_plot_for_patients(top_tfs,patient_list2,log2FC,cohort,group,'G2',outdir)
