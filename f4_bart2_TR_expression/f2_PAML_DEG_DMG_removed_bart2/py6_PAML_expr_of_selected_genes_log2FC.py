...
def expr_plot_for_patients(top_tfs,patients1,patients2,log2FC,cohort,group,outdir):
    ...
    tf_expr_compr_ttest1 = pd.read_csv('f3a_PAML_expr_of_topTR_sep_patient_ttest_rel'+os.sep+'ttest_{}_{}_patientG1.csv'.format(cohort,group),index_col=0)
    tf_expr_compr_ttest2 = pd.read_csv('f3a_PAML_expr_of_topTR_sep_patient_ttest_rel'+os.sep+'ttest_{}_{}_patientG2.csv'.format(cohort,group),index_col=0)
    
    g1_sig_tfs = tf_expr_compr_ttest1[tf_expr_compr_ttest1['pvalue']<.05].index
    g2_sig_tfs = tf_expr_compr_ttest2[tf_expr_compr_ttest2['pvalue']<.05].index
    ...
    for ii in np.arange(len(top_tfs)):
        ...
        # == p<0.05
        sig1_marker.append('*' if gene_name in g1_sig_tfs else '')
        sig2_marker.append('*' if gene_name in g2_sig_tfs else '')
    ...
    barh_pos=[0.,2.,4.,6,8,10]
    
    # ==== label the *
    for pos in positions:
        plt.text(fc_thre*.9,pos+.35,sig1_marker[int(pos)],fontsize=20,color='tab:orange',va='top',weight='bold')
        plt.text(fc_thre*.9,pos-.05,sig2_marker[int(pos)],fontsize=20,color='tab:blue',va='top',weight='bold')
...
# ==== main section
outdir = 'f6_PAML_expr_of_selected_gene_log2FC'
...
for cohort in ["PAML"]:
    ...
    top_tfs = ['RBBP5','WDR5','ASH2L','DPY30','MYB','CEBPA'][::-1]
    ...
    patients1,patients2 = patient_list2_tmp,patient_list1_tmp
    

    # plot the fig    
    box_vals1,box_vals2,positions,gene_names=[],[],np.array([]),[]
    tf_expr_compr_ttest1,tf_expr_compr_ttest2 = pd.DataFrame(),pd.DataFrame()
    df_patient_c1,df_patient_c2 = pd.DataFrame(),pd.DataFrame()
    sig1_marker,sig2_marker=[],[]
    ...
    for ii in np.arange(len(top_tfs)):
        ...
        gene_name = top_tfs[ii] 
        # paired ttest 
        s1,p1 = stats.ttest_rel(tpm_relapse.loc[gene_name][patients1].values,
                                tpm_diagnosis.loc[gene_name][patients1].values,nan_policy='omit')
        tf_expr_compr_ttest1.loc[gene_name,'t-statistic'] = s1
        tf_expr_compr_ttest1.loc[gene_name,'pvalue'] = p1
        
        s2,p2 = stats.ttest_rel(tpm_relapse.loc[gene_name][patients2].values,
                                tpm_diagnosis.loc[gene_name][patients2].values,nan_policy='omit')
        tf_expr_compr_ttest2.loc[gene_name,'t-statistic'] = s2
        tf_expr_compr_ttest2.loc[gene_name,'pvalue'] = p2
    
        
        # == p<0.05
        sig1_marker.append('*' if p1<0.05 else '')
        sig2_marker.append('*' if p2<0.05 else '')
    ...
    # df_patient_c2 = df_patient_c2[df_patient_c2.columns[::-1]]
    # save the df
    df_patient_c1.to_csv('{}/{}_genes_{}_patient_C1.csv'.format(outdir,cohort,'log2FC'))
    df_patient_c2.to_csv('{}/{}_genes_{}_patient_C2.csv'.format(outdir,cohort,'log2FC'))
    tf_expr_compr_ttest1.to_csv('{}/{}_genes_{}_patient_C1.csv'.format(outdir,cohort,'pairedTest'))
    tf_expr_compr_ttest2.to_csv('{}/{}_genes_{}_patient_C2.csv'.format(outdir,cohort,'pairedTest'))
    
    # plot the figs
    face_colors1 = ['tab:orange']*len(top_tfs)
    face_colors2 = ['tab:blue']*len(top_tfs)
    
    figname = '{}/{}_genes_log2FC.pdf'.format(outdir,cohort)
    plt.figure(figsize=(6,4))
    # boxes of patient group1
    g = plt.boxplot(box_vals1,positions=positions+.15,vert=False,widths = .2,patch_artist=True,\
                boxprops=dict(color='k',facecolor='w',fill=True,lw=1),\
    ...
    # ==== label the *
    for pos in positions:
        plt.text(fc_thre*.9,pos+.2,sig1_marker[int(pos)],fontsize=20,color='tab:orange',va='top',weight='bold')
        plt.text(fc_thre*.9,pos-.1,sig2_marker[int(pos)],fontsize=20,color='tab:blue',va='top',weight='bold')
