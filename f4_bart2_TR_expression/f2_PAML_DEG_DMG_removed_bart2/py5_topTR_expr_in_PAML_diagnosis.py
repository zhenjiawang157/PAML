...
def return_patient_info(project_dir,cohort):
    ...
    return np.sqrt(tpm_diagnosis),np.sqrt(tpm_relapse)
...
#         plt.scatter(scatter_x,box_vals[position_id],color=colors[position_id],s=10,zorder=0,alpha=0.99)
    ...
    plt.legend([g["boxes"][0],g["boxes"][1]],['Patient G1','Patient G2'],fontsize=12,borderaxespad=0.2,labelspacing=.2,handletextpad=0.2,handlelength=1,loc=0,frameon=False)
    plt.ylabel('sqrt TPM',fontsize=16)
...
def expr_plot_for_patients(top_tfs,patients1,patients2,tpm_expr,cohort,group,flag,outdir):
    ...
    for ii in np.arange(len(top_tfs)):
        ...
        box_vals.append(tpm_expr.loc[gene_name][patients1].values)
        # relapse
        positions.append(ii+.25)
        box_vals.append(tpm_expr.loc[gene_name][patients2].values)
        gene_names.append(gene_name)
        
    colors = ['whitesmoke','k']*top_tf_num
    figname = '{}/{}_{}_expr_{}.pdf'.format(outdir,cohort,group,flag)
    tf_expr_compr_ttest = compr_boxplot(positions,box_vals,figname,gene_names,colors,cohort,group,flag)
    tf_expr_compr_ttest.to_csv(outdir+os.sep+'ttest_{}_{}_expr_{}.csv'.format(cohort,group,flag))
#     print('cohort {}, #{} {}'.format(cohort,flag,len(patient_list1)))



# ==== main section
outdir = 'f5_PAML_topTR_expr_in_diagnosis_sep_patient'
...
for group in ['div','up','down']:
    for cohort in ["PAML"]:
        ...
        # for patient group 1
        expr_plot_for_patients(top_tfs,patient_list1,patient_list2,tpm_diagnosis,cohort,group,'diagnosis',outdir)

        # for patient group 1
        expr_plot_for_patients(top_tfs,patient_list1,patient_list2,tpm_relapse,cohort,group,'relapse',outdir)
