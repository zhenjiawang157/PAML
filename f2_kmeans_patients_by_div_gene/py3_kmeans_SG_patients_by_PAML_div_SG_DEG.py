...
def main():
    ...
    outdir='f3_kmeans_SG_patients_by_PAML_div_SG_DEG'
    os.makedirs(outdir,exist_ok=True)
    os.makedirs(outdir+os.sep+'csv',exist_ok=True)
    
    file_dir = '/nv/vol190/zanglab/zw5j/wwork2018/AML_fran/f0_fran_data_new/data_processed'
    # == this is DEG of PAML 
    paml_genes1 = [i.strip() for i in open('../f1_kmeans_patients_by_deg/f1_kmeans_PAML_patients_by_PAML_DEG/txt/PAML_pthre5_rk3_ck2_genes_cluster1.txt').readlines()]
    paml_genes2 = [i.strip() for i in open('../f1_kmeans_patients_by_deg/f1_kmeans_PAML_patients_by_PAML_DEG/txt/PAML_pthre5_rk3_ck2_genes_cluster2.txt').readlines()]
    paml_genes3 = [i.strip() for i in open('../f1_kmeans_patients_by_deg/f1_kmeans_PAML_patients_by_PAML_DEG/txt/PAML_pthre5_rk3_ck2_genes_cluster3.txt').readlines()]
    selected_genes = paml_genes1+paml_genes2+paml_genes3
    # PAML div genes for patient clustering
    target_genes = paml_genes3
