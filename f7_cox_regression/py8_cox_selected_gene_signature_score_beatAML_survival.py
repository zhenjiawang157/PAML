...
# == main ==
outdir='f8_cox_selected_genes_beatAML_survival'
...
top_tfs_PAML= set(['JMJD1C','MLL','TEAD4','ERG','ZBTB7A','ETS1'])
top_tfs= set(['JMJD1C','KMT2A','TEAD4','ERG','ZBTB7A','ETS1'])

# prepare the matrix for cox regression
expression_data = np.transpose(tpm_df.loc[top_tfs_PAML])
...
beatAML_clinical_df = beatAML_clinical_df[['isRelapse','vitalStatus','overallSurvival']].dropna()
# beatAML_clinical_df = pd.read_csv('{}/f1_clinical_info/selected_case_id_with_clinical_info.csv'.format(beatAML_dir),index_col=0) 
# beatAML_clinical_df = beatAML_clinical_df.dropna()
...
beatAML_data=np.transpose(beatAML_expression_df.loc[top_tfs])
...
# beatAML_clinical_df[beatAML_clinical_df['isRelapse']==False]['overallSurvival'].mean()
# beatAML_clinical_df[beatAML_clinical_df['isRelapse']==True]['overallSurvival'].mean()


# prepare the matrix for cox regression
cph_data = pd.concat([beatAML_clinical_df.loc[survival_df.index]['overallSurvival'],beatAML_data.loc[survival_df.index]],axis=1)
# cph_data = pd.concat([clinical_df,expression_data],axis=1)

# add penalizer term to the cox regression 
cph = CoxPHFitter()
# cph = CoxPHFitter(penalizer=0.1, l1_ratio=1.0) # sparse solutions,
cph.fit(cph_data, duration_col='overallSurvival')
# print(cph.summary)
