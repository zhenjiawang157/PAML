...
# == main ==
outdir='f6_cox_on_topTRs_beatAML_survival'
...
for gene_num in [9,10,20,30,33]:
    beatAML_data=np.transpose(beatAML_expression_df.loc[top_tfs[:gene_num]].dropna()) # gene on the column
    cph_data = pd.concat([beatAML_clinical_df,beatAML_data],axis=1)
    for penalizer in penalizers[:]:
        for l1_ratio in l1_ratios[:]:
            basename = 'TR{}_Cox_penalizer{}_l1_ratios{}'.format(gene_num,penalizer,l1_ratio)
